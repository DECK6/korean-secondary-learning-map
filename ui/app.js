...
function renderCourseDetail(detail) {
  ...
  const renderStandards=()=>{
    ...
    $('#standard-list').innerHTML=standards.length?standards.map((standard)=>{const topics=topicsByStandard.get(standard.id)??[],domain=domainById.get(standard.domainId),primary=topics.filter((topic)=>topic.topicRole!=='auxiliary'),auxiliary=topics.filter((topic)=>topic.topicRole==='auxiliary'),labelById=new Map(topics.map((topic)=>[topic.id,topic.labelKorean])),collapsed=auxiliary.length?`<details class="collapsed-topics"><summary>축약 주제 ${number(auxiliary.length)}개 — 대표 주제로 대신합니다</summary>${auxiliary.map((topic)=>`<p><strong>${escapeHtml(topic.labelKorean)}</strong><br>대신 제시: ${escapeHtml(labelById.get(topic.collapseInto)??topic.collapseInto??'대표 주제')}<br>${escapeHtml(topic.evidence?.[0]??'')}</p>`).join('')}</details>`:'';return `<details class="standard"><summary><code>${escapeHtml(standard.code)}</code> ${escapeHtml(standard.summary)}</summary><div class="standard__body"><p><strong>영역</strong><br>${escapeHtml(domain?.labelKorean??'영역 미확인')}</p><p><strong>세부 주제 ${number(primary.length)}개</strong><br>${primary.map((topic)=>`${escapeHtml(topic.labelKorean)}${topic.contentKind==='source-grounded-draft'?' <span class="content-badge">검토 초안</span>':''}`).join('<br>')||'연결 주제 없음'}</p><p><strong>관찰 증거</strong><br>${primary.map((topic)=>escapeHtml(topic.evidence?.[0]??'')).join('<br>')}</p><p><strong>평가 질문</strong><br>${primary.map((topic)=>escapeHtml(topic.assessmentPrompts?.[0]??'')).join('<br>')}</p>${collapsed}<p class="source-line">PDF p.${standard.sourceLocator.pdfPage} · ${escapeHtml(standard.sourceLocator.sourceId)} · SHA-256 ${escapeHtml(standard.sourceLocator.sha256.slice(0,16))}… · 기계적 파생 요약</p></div></details>`}).join(''):'<p class="empty-message">일치하는 성취기준이 없습니다.</p>';
