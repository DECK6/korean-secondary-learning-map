...
function renderTransitions() {
  ...
  $('#transition-list').innerHTML=rows.length?rows.map((item)=>`<article class="transition"><div class="transition__node"><small>중학교 · ${escapeHtml(item.from.groupLabel)}</small><strong>${escapeHtml(item.from.courseLabel)}</strong><span>${escapeHtml(item.from.label)}</span></div><div class="transition__edge"><span>${transitionLabels[item.kind]??item.kind}</span><b>→</b><small>후보</small></div><div class="transition__node"><small>고등학교 과목</small><strong>${escapeHtml(item.to.courseLabels.join(', '))}</strong><span>${escapeHtml(item.to.topicLabel)}</span></div></article>`).join(''):'<p class="empty-message">조건에 맞는 전이가 없습니다.</p>';
}
