...
| 구분 | 초등 `kr-full-depth-v0.4` | 중학교 `v0.3` |
| --- | ---: | ---: |
| 성취기준 | 620 | 714 |
| 세부 주제 | 1,956 | 2,160 |
| 기준당 주제 | 3.15 | 3.03 |
...
초등은 교과별로 기준당 2~5개 주제를 생성한다. 중학교도 같은 밀도 원칙을 적용해 국어 4개, 도덕 5개, 기술·가정/정보 2개, 나머지 과목 3개 후보로 분해한다. 기존 714개 주제 ID는 `standard-core`로 보존하고 1,446개 `subject-facet` 주제를 추가했다. 선수 관계 수는 맞추지 않는다. 초등 관계도 모델 상대적인 추천이며, 중학교는 공식 근거나 식별 가능한 전문가 검토가 생기기 전까지 0건을 유지한다.
