...
현재 릴리스는 `v0.2.0-candidate`다. NCIC 공식 PDF 38개에서 확인한 714개 중학교 성취기준과 50,749개 고등학교 성취기준을 과목·영역(`Domain`)·PDF 해시·페이지 locator에 연결한다. 공식 원문은 포함하지 않는다. 세부 주제·중→고 전이 398건·과목 관계·예시 경로는 저장소가 만든 검토 후보이며 공식 필수 요건이나 교과 전문가 승인을 뜻하지 않는다. 근거 없는 코드 배열 순서로 만든 선수 관계는 전부 제거했다.
...
- `dist/ontology`: 166,880개 노드·1,798,025개 트리플의 결정적 JSON-LD/Turtle ABox와 manifest
...
- 기계적 파생 요약·세부 주제·전이·과목 관계·경로는 모두 `candidate` 또는 `repository-authored`로 구분한다.
...
- [권리 고지](NOTICE.md)
- [후보 릴리스 보고서](docs/release/v0.2.0-candidate.md)
