...
async function build() {
  ...
  for (const profile of ['middle', 'high']) {
    ...
    for (const record of data[profile].topics) pushNode(graph, ttl, {
      id: record.id,
      json: { '@id': jsonIri(record.id), '@type': 'slm:LearningTopic', label: record.labelKorean, description: record.description, topicInCourse: refs(record.courseIds), inDomain: { '@id': jsonIri(record.domainId) }, alignsToStandard: refs(record.standardAlignments, 'standardId'), alignmentKind: record.standardAlignments.map((a) => a.alignmentKind), basis: record.standardAlignments.map((a) => a.basis), topicType: record.types, evidence: record.evidence, assessmentPrompt: record.assessmentPrompts, hasSource: refs(record.sourceRefs), verificationStatus: record.verificationStatus, reviewStatus: record.reviewStatus },
      triples: [`a slm:LearningTopic`, `rdfs:label ${ko(record.labelKorean)}`, `slm:description ${ko(record.description)}`, ...record.courseIds.map((id) => `slm:topicInCourse ${iri(id)}`), `slm:inDomain ${iri(record.domainId)}`, ...record.standardAlignments.map((a) => `slm:alignsToStandard ${iri(a.standardId)}`), ...record.standardAlignments.map((a) => `slm:alignmentKind ${literal(a.alignmentKind)}`), ...record.standardAlignments.map((a) => `slm:basis ${literal(a.basis)}`), ...record.types.map((value) => `slm:topicType ${literal(value)}`), ...record.evidence.map((value) => `slm:evidence ${ko(value)}`), ...record.assessmentPrompts.map((value) => `slm:assessmentPrompt ${ko(value)}`), ...record.sourceRefs.map((id) => `slm:hasSource ${iri(id)}`), `slm:verificationStatus ${literal(record.verificationStatus)}`, `slm:reviewStatus ${literal(record.reviewStatus)}`],
    });
...
if (checkOnly) {
  ...
  await atomicWrite(join(outDir, 'manifest.json'), `${JSON.stringify({ version: '0.2.0-candidate', graphNodeCount: built.graphCount, artifacts }, null, 2)}\n`);
