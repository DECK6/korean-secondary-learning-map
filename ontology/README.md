...
`bun run build:ontology`는 정규화 JSON에서 결정적 ABox를 `dist/ontology/learning-map.{jsonld,ttl}`로 만든다. `bun run validate:ontology`는 168,326개 노드·1,879,104개 트리플의 JSON-LD↔Turtle 전체 동형성, bounded RDFS/OWL-RL subset과 적대 fixture 탐지를 검증한다. 중학교의 2,160개 주제는 `MiddleLearningTopic`으로 타입화하며 `standard-core`/`subject-facet`과 facet key를 보존한다. 이때 `CourseRelation`이 `TransitionAlignment`로 잘못 추론되지 않는지도 회귀 검사한다. `bun run validate:shacl`은 PySHACL Advanced로 양성 fixture 통과, 적대 fixture 거절, 전체 ABox 적합성을 확인한다. `bun run validate:sparql`은 SCQ-01~20을 전체 ABox에서 실제 실행해 고정된 예상 결과와 비교한다.
