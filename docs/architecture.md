...
| `MiddleLearningTopic` | 중학교 성취기준의 핵심·과목별 facet 주제 | `standard-core`와 `subject-facet`을 구분하고 기존 핵심 ID를 보존 |
...
- 초등과 비교 가능한 깊이를 위해 성취기준당 2~5개 주제 후보를 만들되, 과목별 분해 정책·facet key·정렬 역할을 기록하고 선수 관계를 자동 추론하지 않는다.
...
접근일: 2026-07-13. 자료별 저작권·공공누리 조건은 실제 원문 인벤토리 단계에서 문서 단위로 다시 기록한다.
